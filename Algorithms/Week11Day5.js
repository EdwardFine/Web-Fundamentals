    ...
    for (let i = 0; i < songDurations.length - 1; i++) {
        for (let j = i + 1; j < songDurations.length; j++) {
            if (songDurations[i] + songDurations[j] === busDuration - 30) {
                if (songDurations[longest[0]] > songDurations[longest[1]]) {
                    if (songDurations[i] > songDurations[longest[0]] || songDurations[j] > songDurations[longest[0]]) {
                        longest = [i, j];
                    }
                } else {
                    if (songDurations[i] > songDurations[longest[1]] || songDurations[j] > songDurations[longest[1]]) {
                        longest = [i, j];
                    }
                }
            }
